<%-- 
    Document   : PolDetails
    Created on : Jun 5, 2021, 7:43:47 AM
            ...
            <div class="w-full h-screen overflow-x-hidden border-t flex flex-col justify-between">
                <main class="w-full p-6">
                    <div class="flex">
                        ...
                        <h1 class="text-3xl text-black pb-6">Poll Overview</h1>
                    </div>
                            ...
                            <div class="w-full leading-loose">
                                <div class="px-10 pt-5 pb-10 bg-white rounded shadow-xl">
                                    <div class="mb-5 flex justify-end">
                                            ...
                                            <div x-data="{ showModal : false }">
                                                ...
                                                <div x-show="showModal" class="fixed text-gray-500 flex items-center justify-center overflow-auto z-50 bg-black bg-opacity-40 left-0 right-0 top-0 bottom-0" x-transition:enter="transition ease duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease duration-300" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
                                                    ...
                                                    <div x-show="showModal" class="bg-white rounded-xl shadow-2xl p-6 sm:w-10/12 mx-10" @click.away="showModal = false" x-transition:enter="transition ease duration-100 transform" x-transition:enter-start="opacity-0 scale-90 translate-y-1" x-transition:enter-end="opacity-100 scale-100 translate-y-0" x-transition:leave="transition ease duration-100 transform" x-transition:leave-start="opacity-100 scale-100 translate-y-0" x-transition:leave-end="opacity-0 scale-90 translate-y-1">
                                                        ...
                                                        <!-- Body -->
                                                        <form action="<%=request.getContextPath()%>/PollController" method="POST">
                                                            <br>
                                                            <input type="hidden" name="pollid" value="<%=poll.getPollid()%>">
                                                            <div>
                                                                <label class="font-semibold">Poll Name</label>
                                                                <input type="text" name="pollName" required class="mb-5 w-full py-1 px-3 rounded-lg border-2 border-blue-300 mt-1 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent" value="<%=poll.getPollName()%>">
                                                            </div>
                                                            <div>
                                                                <label class="font-semibold">Poll Description</label>
                                                                <input type="text" name="pollDesc" required class="w-full py-1 px-3 rounded-lg border-2 border-blue-300 mt-1 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent" value="<%=poll.getPollDesc()%>">
                                                            </div>

                                                            <!-- Buttons -->
                                                            <div class="flex justify-end space-x-5 mt-5">
                                                                <button type="submit" name="action" value="updatePoll" class="px-4 py-2 text-sm bg-white rounded-xl border transition-colors duration-150 ease-linear border-gray-200 text-gray-500 focus:outline-none focus:ring-0 font-bold hover:bg-gray-50 focus:bg-indigo-50 focus:text-indigo">
                                                                    Update
                                                                </button>
                                        ...
                                        <form action="<%=request.getContextPath()%>/PollController">
                                            <input type="hidden" name="pollid" value="<%=poll.getPollid()%>">
                                            <button type="submit" name="action" value="deletePoll" class="px-4 py-1 text-red-700 font-light tracking-wider hover:bg-gray-200 rounded hover:text-red-700 focus:outline-white">
                                    ...
                                    <div>
                                        <table class="w-full text-left border-collapse border border-gray-500">
                                            <tr>
                                                ...
                                                <td class="border border-gray-500 pl-4"><%=poll.getPollName()%></td>
                                            </tr>
                                            <tr>
                                                <th class="border border-gray-500 p-4"><label>Poll Description</label></th>
                                                <td class="border border-gray-500 pl-4"><%=poll.getPollDesc()%></td>
                                            </tr>
                                        </table>
                                    </div>
                                </div>
                    ...
                    <div class="w-full mt-12">
                        ...
                        <div class="flex">
                            ...
                            <div class="ml-5">
                                ...
                                <div x-data="{ showModal : false }">
                                    <!-- Button -->
                                    <button @click="showModal = !showModal" class="flex items-center px-4 py-1 text-white font-light tracking-wider bg-gray-900 rounded hover:bg-gray-700">
                                    ...
                                    <div x-show="showModal" class="fixed text-gray-500 flex items-center justify-center overflow-auto z-50 bg-black bg-opacity-40 left-0 right-0 top-0 bottom-0" x-transition:enter="transition ease duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease duration-300" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
                                        ...
                                        <div x-show="showModal" class="bg-white rounded-xl shadow-2xl p-6 sm:w-10/12 mx-10" @click.away="showModal = false" x-transition:enter="transition ease duration-100 transform" x-transition:enter-start="opacity-0 scale-90 translate-y-1" x-transition:enter-end="opacity-100 scale-100 translate-y-0" x-transition:leave="transition ease duration-100 transform" x-transition:leave-start="opacity-100 scale-100 translate-y-0" x-transition:leave-end="opacity-0 scale-90 translate-y-1">
                                            ...
                                            <!-- Body -->
                                            <form action="<%=request.getContextPath()%>/PollItemController" method="POST" enctype="multipart/form-data">
                                                <div>
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                            <div class=" flex justify-between">
                                                        ...
                                                        <div x-show="showModal" class="fixed text-gray-500 flex items-center justify-center overflow-auto z-50 bg-black bg-opacity-40 left-0 right-0 top-0 bottom-0" x-transition:enter="transition ease duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease duration-300" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
                                                            ...
                                                            <div x-show="showModal" class="bg-white rounded-xl shadow-2xl p-6 sm:w-10/12 mx-10" @click.away="showModal = false" x-transition:enter="transition ease duration-100 transform" x-transition:enter-start="opacity-0 scale-90 translate-y-1" x-transition:enter-end="opacity-100 scale-100 translate-y-0" x-transition:leave="transition ease duration-100 transform" x-transition:leave-start="opacity-100 scale-100 translate-y-0" x-transition:leave-end="opacity-0 scale-90 translate-y-1">
                                                                ...
                                                                <form action="<%=request.getContextPath()%>/PollItemController" method="POST" enctype="multipart/form-data">
                                                                    ...
                                                                    <br>
                                                                    <input type="hidden" name="pollitemid" value="<%=pollItem.getPollitemid()%>">
                                                                    <input type="hidden" name="pollid" value="<%=poll.getPollid()%>">
                                                                    <div>
                                                                        <label class="sr-only">Poll Choice</label>
                                                                        <input type="text" name="pollItemName" required class="w-full py-1 px-3 rounded-lg border-2 border-blue-300 mt-1 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent" value="<%=pollItem.getPollItemName()%>">
                                                                    </div>
                                                ...
                                                <form action="<%=request.getContextPath()%>/PollItemController">
                                                    <input type="hidden" name="pollitemid" value="<%=pollItem.getPollitemid()%>">
                                                    <input type="hidden" name="pollid" value="<%=pollid%>">
                                                    <button type="submit" name="action" value="deletePollItem" class="px-4 py-1 text-red-700 font-light tracking-wider hover:bg-gray-200 rounded hover:text-red-700 focus:outline-white">
                                                        <i class="fa fa-trash" aria-hidden="true"></i>
                                                    </button>  
                                                </form>                                    
                                            </div>
