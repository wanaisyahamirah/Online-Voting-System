<%-- 
    ...
    Created on : Jul 10, 2021, 5:06:19 AM
    Author     : Hp
--%>

<%@page import="Model.Contact"%>
<%@page import="DAOImpl.ContactDAOImpl"%>
...
<html lang="en">
    <head>
        ...
        <title>Message Details | Voting Rush</title>
    ...
    <body class="bg-gray-100 font-family-karla flex">
        ...
        <nav class="text-white text-base font-semibold pt-3">
            ...
            <a href="Message.jsp" class="flex items-center active-nav-link text-white py-4 pl-6 nav-item">
    ...
    <div class="w-full flex flex-col h-screen overflow-y-hidden">
        ...
        <header x-data="{ isOpen: false }" class="w-full bg-sidebar py-5 px-6 sm:hidden">
            ...
            <nav :class="isOpen ? 'flex': 'hidden'" class="flex flex-col pt-4">
                ...
                <a href="Message.jsp" class="flex items-center active-nav-link text-white py-2 pl-4 nav-item">
                    <i class="fas fa-envelope mr-3"></i>
                    Messages
                </a>
            ...
            <div class="w-full h-screen overflow-x-hidden border-t flex flex-col justify-between">
                <main class="w-full p-6">
                    <div class="flex">
                        <a class="text-3xl text-blue-500 pb-6 hover:underline" href="Message.jsp">Message</a>
                        <span class="text-3xl text-black pb-6 mx-3"> / </span>
                        <h1 class="text-3xl text-black pb-6">Message Details</h1>
                    </div>

                    <%
                        String contactid = request.getParameter("id");
                        ContactDAO contactDAO = new ContactDAOImpl();
                        Contact selectedContact = contactDAO.retrieveContactByContactId(Integer.parseInt(contactid));
                                ...
                                <div class="px-10 pt-5 pb-10 bg-white rounded shadow-xl">
                                    <div class="text-right">
                                        <form action="<%=request.getContextPath()%>/ContactController" method="POST">
                                            <input type="hidden" name="contactid" value="<%=selectedContact.getId()%>">
                                    ...
                                    <div>
                                        <table class="w-full text-left border-collapse border border-gray-500">
                                            <tr>
                                                <th class="border border-gray-500 p-4"><label>Full Name</label></th>
                                                <td class="border border-gray-500 pl-4"><%=selectedContact.getFullname()%></td>
                                            </tr>
                                            <tr>
                                                <th class="border border-gray-500 p-4"><label>Email</label></th>
                                                <td class="border border-gray-500 pl-4"><%=selectedContact.getEmail()%></td>
                                            </tr>
                                            <tr>
                                                <th class="border border-gray-500 p-4"><label>Subject</label></th>
                                                <td class="border border-gray-500 pl-4"><%=selectedContact.getSubject()%></td>
                                            </tr>
                                            <tr>
                                                <th class="border border-gray-500 p-4"><label>Description</label></th>
                                                <td class="border border-gray-500 pl-4"><%=selectedContact.getDescription()%></td>
                                            </tr>
                                        </table>
                                    </div>
                                </div>
